...
- What constitutes a "concept" vs "entity" vs "method" vs "application" in this domain

This ensures you create pages in the correct categories and use the right terminology from the start.
...
- Wiki already has substantial content (5+ source summaries with linked concept/entity pages) — the wiki is past cold-start; continue with ingest
...
- `raw/` has 5+ files and wiki is empty or minimal (fewer than 3 source summaries)
...
2. **Quick-read each source** — For each raw document, do a shallow scan to extract:
   - Title and authors
   - 3-5 key concepts/terms mentioned
   - 1-2 key entities (models, datasets, platforms, labs)
...
3. **Compile a landscape map** — Present the user with a structured summary of what you found, using the categories defined in `CLAUDE.md`:
   ```
   ## Raw Sources Landscape

   | # | Source ID | Title | Key Concepts | Key Entities | Domain |
   |---|-----------|-------|--------------|--------------|--------|
   | 1 | ... | ... | ... | ... | ... |
   ...
   (organized by CLAUDE.md category definitions)
   ```

### Phase 2: Discuss

4. **Discuss with user** — Ask:
   - Which concepts/entities are most important to them? (This guides page depth)
   - Should any concepts be merged or split differently than suggested?
...
6. **Write source summaries** — For each source (or each source the user prioritized), write `wiki/sources/{source-id}.md` with frontmatter and a structured summary. These are lighter than ingest-level summaries — capture the essentials (what, why, key findings) rather than exhaustive detail. Include English quotes where original phrasing matters. Format:
   ...
   Summary structure (in the language specified by CLAUDE.md):
   - 一句话概括
   - 核心贡献/发现
   - 关键概念与实体（with [[wikilinks]])
   - 与其他源的关系（if apparent）

7. **Create category pages** — For each page identified in Phase 1+2, create it in the directory defined by `CLAUDE.md` for its category:
   ...
   - Add frontmatter with all relevant source IDs in `sources` field
...
8. **Write overview** — Compose `wiki/overview.md` as a high-level synthesis of the field based on all scanned sources. This is the panoramic view that individual pages zoom into. Structure:
...
9. **Build index** — Write `wiki/index.md` listing all pages created, organized by category with one-line summaries (language per CLAUDE.md) and wikilinks. Every page must appear in the index.

10. **Append log** — Add entry to `wiki/log.md`:
    ```
    ## [YYYY-MM-DD] init | Batch initialization from N sources

    Scanned N sources in raw/, discussed priorities with user, created:
    - M source summaries
    - K concept/entity/method/application pages
    - overview.md, index.md
    ```

### Phase 4: Verify

11. **Consistency check** — Quickly verify:
...
12. **Present results to user** — Show what was created and suggest next steps:
...
| Depth per source | Shallow scan + essentials | Deep reading + discussion |
...
| Creating stub pages with just a one-line definition | Pages should synthesize across sources, not just define. Each concept page should mention which sources discuss it, what approaches they take, and link to related concepts |
| Processing sources sequentially instead of by topic | Init's power is cross-source synthesis. Write concept pages that pull from ALL relevant sources at once, not one-by-one |
...
| Forgetting to update frontmatter sources field | Every concept/entity page must list ALL source IDs that contributed to it |
